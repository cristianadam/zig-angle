...
Cross-compiles [ANGLE](https://chromium.googlesource.com/angle/angle) with
`zig cc` / `zig c++`, driven by CMake. Upstream ANGLE is carried as a submodule
and serves every target; WebKit's vendored copy at `Source/ThirdParty/ANGLE`
also works, if you have a checkout and point `ANGLE_SOURCE_DIR` at it.

The zig toolchain shim underneath is [zig-cross](https://github.com/mrexodia/zig-cross)
by mrexodia — somebody else's project, carried as a submodule too.
...
### Qt Quick: qtshadertools and qtdeclarative

Nothing in this toolchain had to change for these. Given a host Qt and an
installed target Qt, both modules cross-compile with zig cc the same way
...
under `#if QT_CONFIG(vulkan) && __has_include(<vulkan/vulkan.h>)`. The guards
disagree, so a module built where the Vulkan headers are not on the include
path compiles a call to something that was never declared. Our Windows qtbase
has Vulkan on with headers borrowed from the ANGLE checkout, and that path is
...
collides with it on a case-insensitive filesystem and the link fails with
...
| Job | Runner | Targets |
| --- | ------ | ------- |
| `cross` | `ubuntu-24.04`, `ubuntu-24.04-arm` | the Linux and Windows triples, plus the Vulkan and X11 variants |
| `macos` | `macos-14` | `aarch64-macos-none`, `x86_64-macos-none` |
| `windows-run` | `windows-2022`, `windows-11-arm` | runs the smoke test out of the artifacts the first job built |

zig cross-compiles, so the Windows targets are built on Linux too, and each job
is just `cmake --workflow --preset <triple>` after `tools/prepare-angle.sh`.
The runner architecture is matched to the target wherever one exists: the
aarch64 Linux presets build on `ubuntu-24.04-arm`, which keeps
`tools/make-linux-sysroot.sh` on its native path and lets the smoke test
actually execute. That script needs no root, which is what lets it work on a
...
The smoke test runs wherever the target can execute, which for most targets is
in the build job itself, during `ctest`: the Linux runners run their own
architecture's glibc triples, and the macOS runner runs `aarch64-macos-none`.
Windows is the one case that needs a second job, because those binaries are
built on Linux and nothing there can run them - hence `windows-run`, which
takes the artifacts and executes them on x86_64 and arm64 Windows. The
alternative would be building them on Windows runners, but cross-compiling
Windows from a Linux host is its own code path worth exercising: the mingw
import libraries are generated by `zig lib` there, and the resource compiler
goes through a shell script rather than a `.cmd`.

Three targets are built but not run, each for a reason that is not going to
change:

* the two musl triples, because a musl dynamic binary needs musl's loader and
  the runner does not have one;
* `x86_64-macos-none`, built on an arm64 runner. Rosetta would run it, but it
  is an optional install, and assuming it turns "cannot run this here" into a
  failed test rather than a skipped one. Windows is the opposite case - arm64
  Windows always emulates x86 and x86_64 - so there the 32-bit build is run on
...
**Checking out ANGLE is not recursive**, and that is load-bearing. ANGLE has
79 submodules of its own - its gclient DEPS mirrored as gitlinks - and one of
them, `third_party/gles1_conform`, is on `chrome-internal.googlesource.com`,
which no public runner can read:

```
fatal: could not read Username for 'https://chrome-internal.googlesource.com'
fatal: clone of '.../angle/es-cts' into submodule path '.../gles1_conform' failed
```

So the workflow asks for `submodules: true` rather than `recursive`, and
`prepare-angle.sh` fetches the five dependencies that are actually needed into
those otherwise empty directories. Git still reports all 79 as uninitialised
afterwards, which is why the submodule does not show up as dirty.

**The workflow is not verified green.** Actions cannot be run from here, so it
is written from what the builds need. Its first run failed exactly as above on
the recursive checkout; the next on two more things that only a machine without
this one's directory layout would notice - the `tools` scripts were committed
without their executable bit, and `ZIG_CROSS_DIR` still defaulted to a path on
the author's disk. zig-cross is a submodule now for the same reason ANGLE is. What is checked locally is what it rests on: a non-recursive
submodule checkout, `prepare-angle.sh` against it including the pinned
converter download and the patch applying, and
`cmake --workflow --preset x86_64-linux-gnu-vulkan` with nothing else
configured, which finds the submodule by itself and passes. The Linux-host
branch that decides whether the smoke test can run was checked the same way,
under WSL: configuring for `aarch64-linux-gnu` on an aarch64 Linux host
registers `angle.smoke` against the built binary, which is what makes the
Linux jobs test rather than merely compile.
...
| `ZIG_CROSS_DIR`             | zig-cross checkout (default: the `third_party` submodule)    |
